...
- 主显示栏以物品详情为最高优先级，不被导航、拍照、识别或记忆过程状态覆盖
- 熄屏唤醒后分阶段重建原生 `scroll-view`，并从本地存储重新水合最近物品缩略列表
...
npx --yes --package=node@20 --package=@yodaos-pkg/aix-cli \
  aix pack . -o MagicGlass-v0.6.5.aix
```
...
```bash
npx --yes --package=node@20 --package=@yodaos-pkg/aix-cli \
  aix list MagicGlass-v0.6.5.aix
```
